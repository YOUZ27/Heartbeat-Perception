...
Heartbeat Perception 是一款让 AI Agent 基于从海量金融数据中，挖掘出宏观事件发展趋势的开源 Skill。
...
Heartbeat-Perception 把这个洞察变成了一个可执行的工具。它接入了 14 个权威金融数据源 — **从 Polymarket 和 Kalshi 这样的预测市场，到美国国债收益率曲线、CFTC 机构持仓、SEC 内部人交易、各国央行利率、加密衍生品、CNN Fear & Greed 市场情绪、CME FedWatch 利率概率。**
...
只要有市场在定价这件事，Heartbeat Perception 就能给出一个基于交易数据的概率估计。
...
| Yahoo Finance | 股票/ETF/外汇/商品 | 价格历史和趋势 |
| Deribit | 加密衍生品 | 期货 term structure、期权 IV |
| US Treasury | 国债收益率 + 汇率 | 利率曲线、通胀预期、财政汇率 |
| CFTC COT | 期货持仓 | 机构仓位方向（smart money） |
| CoinGecko | 加密现货 + 全球概览 | BTC/ETH 价格、市值、BTC 主导率、市场排名 |
| SEC EDGAR | 内部人交易 + 全文检索 | Form 4 买卖信号、关键词公告搜索 |
| BIS | 央行数据 | 政策利率、信贷/GDP 缺口 |
| World Bank | 发展指标 | GDP、人口、贸易 |
| Yahoo Finance Options | US 期权链 + Greeks | IV、delta/gamma/theta/vega、put/call ratio、max pain、IV skew |
| CNN Fear & Greed | 市场情绪 | 7 个价格信号合成 → 0-100 恐惧贪婪评分 |
| CME FedWatch | 利率概率 | 联邦基金利率期货隐含 FOMC 利率变化概率 |
| Web Search | 网页搜索 + 页面抓取 | VIX、CDS 等补充数据、任意网页正文提取 |
...
> 安装这个开源项目并读取 SKILL.md 作为你的工作指令：https://github.com/YOUZ27/Heartbeat-Perception
...
- 14 个数据源中有 12 个零外部依赖（纯 Python 标准库）。价格历史和期权链分析需要额外安装：
...
│   ├── http.py             # HTTP 客户端抽象
│   ├── combination.py      # 概率合成（线性池/对数池）
│   ├── signal_quality.py   # 信号质量评估框架
│   ├── snapshots.py        # HTTP 响应录制/回放（测试用）
│   └── providers/          # 14 个数据 provider
├── references/             # API 速查
...
- **零依赖优先** — 12/14 个 provider 只用 Python 标准库，无需 `pip install`
